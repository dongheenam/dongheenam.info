...
This is where it becomes interesting: because the _definition_ of $\sqrt{a}$ is the positive number whose square is $a$, and $\sqrt{2}+\sqrt{3}$ squared is $5+2\sqrt{6}$, we can say $$ \textstyle{\sqrt{5+2\sqrt{6}}} = \sqrt{2}+\sqrt{3}. $$
...
Below are two more examples. Note there is nothing in front of $\sqrt{ab}$ -- think of a way to _create_ a factor of $2$.
...
So $\sqrt{(x-1)^2}$ is _not_ equal to $x-1$ in general! Always check whether the expression inside is positive, or not!
...
People often forget $\sqrt{(\cdots)}$ is _always_ positive, as long as it is a real number. And you should be able to tell, the answers to the example questions above are all positive. So this can be used as a check -- if you remove the square root and get something negative, something is going wrong!
