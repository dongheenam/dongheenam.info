...
We call $a$ the _base_ and $m$ the _index_ (or exponent, power).
...
{{% note Definition %}}
$a^2$ is read as $a$ _squared_. $a^3$ is read as $a$ _cubed_.
...
{{% note Definition %}}
The _'dot' symbol_ $\boldsymbol{\cdot}$ means multiplying. In other words,
$$ g\times h = g\cdot h = g h. $$

$g\cdot h$ is read as either _$g$ dot $h$_, or simply $g$ $h$.
...
{{% note Definition %}}
_Expanding_ means to express many terms and polynomials multiplied together as a single polynomial. For example,
