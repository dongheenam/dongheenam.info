...
Translating a graph means to move it around. Because there are two axes on the coordinate plane, there are two directions (horizontally and vertically) we can move the graph to!
...
Some of you might be thinking that this rule is counter-intuitive. For example, if you want to move a point, say $A(0,1)$, to the right by three units, you have to *add* three to the x-coordinates (so it becomes $A'(3, 1)$), but if you want to move $y=3x$ to the right by three units, you have to *subtract* three from $x$!

Here comes the secret: you are not actually moving the graph, but you are changing the perspective by moving the *coordinate plane itself*. So, if you want to move your graph up, you move the coordinate plane down instead to achieve the same effect.
...
If you find this too confusing, please do not worry! It is a very confusing concept (I took several years to finally understand it) and you certainly do not have to memorise this. However, it is a little challenge for those who always seek reasons!
...
Reflecting a graph is easy - all we need is a negative sign!
...
From the previous lesson, we discussed the general form of a linear function is $$ f(x) = ax + b. $$ It is easy to tell that you can obtain this graph by vertically stretching the graph of $y=x$ by a factor of $a$, and then moving it up by $b$:
...
So, how do these operations relate with the property of $y=ax+b$? Well, the graph of $y=x$ has the gradient of $1$ and the y-intercept of $0$. Because you vertically stretch the graph by a factor of $a$ first, its gradient also increases by the same factor, while fixing the y-intercept. Then you move the graph upward by $b$, so the y-intercept also move up by $b$, while fixing the gradient. Therefore, the graph of $y=ax+b$ has the gradient of $a$, and the y-intercept of $b$.

### Intercept form

Note that the graph of $x+y=1$ has the x-intercept of $1$ and the y-intercept of $1$. How can we make this graph to have x-intercept of $a$ and y-intercept of $b$? Well, the answer is dilation: you need to vertically stretch the graph by $b$ (to move the y-intercept), and horizontally stretch the graph by $a$ (to move the x-intercept):
