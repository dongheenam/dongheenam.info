...
- if you earn less than \\$18 200 per year, then you pay \\$0;
- if you earn more than \\$18 200 but less than \\$45 000 per year, then you pay 19c for every dollar you make beyond \\$18 200;
- if you earn more than \\$45 000 but less than \\$120 000 per year, then you pay \\$5092 plus 32.5c for every dollar you make beyond \\$45 000;
- and so on.
...
{{% note Example %}} Let \\$$I$ be the annual income and $T$ the amount of income tax you need to pay as an Australian resident. Then
$$ T = 
\begin{cases}
0 & \text{if $I \le 18200$,} \\\\
...
1. Note the domain of $f$ is $0\le x \le 4$. The graph of $y=f(x)$ is

{{< figure src="courses/jpn-maths-1/functions/functions-and-graphs/piecewise-ex2a.png" alt="Graph of y = f(x)." >}}

2. If substitute $x$ with $f(x)$, the definition of $f(x)$ becomes
$$ f(f(x)) = \begin{cases}
  2f(x) & \text{for $0\le f(x) < 2$,} \\\\
  8-2f(x) & \text{for $2\le f(x) \le 4$.}
\end{cases} $$
From the graph of $y=f(x)$, we find: <br><br>
$0\le f(x) < 2$ when $0 \le x < 1$ or $3 < x \le 4$, and<br>
$2\le f(x) \le 4$ when $1 \le x \le 3$.<br><br>
Then, there are four different rules for $y = f(f(x))$:<br>
When $0\le x < 1$, $f(x) = 2x$ and $$ f(f(x)) = 2f(x) = 2 \cdot 2x = 4x. $$
When $1\le x < 2$, $f(x) = 2x$ and $$ f(f(x)) = 8-2f(x) = 8 - 2\cdot 2x = 8 - 4x. $$
When $2\le x < 3$, $f(x) = 8-2x$ and $$ f(f(x)) = 8-2f(x) = 8-2(8-2x) = 4x-8. $$
When $3\le x < 4$, $f(x) = 8-2x$ and $$ f(f(x)) = 2f(x) = 2(8-2x) = 16-4x. $$
...
{{% note Question %}} Try [Practice Questions](#practice-questions) 1--3.
...
{{% note Theorem %}} $\lfloor x \rfloor = n$ if and only if $n$ is an integer and $n \le x < n+1$.
...
The graph of $y = 2\lfloor x \rfloor$ and the graph of $y = \lfloor 2x \rfloor$ sounds pretty similar, but there is one important difference: the latter function rounds the number *after* the multiplication. Thus, the values of $y$ become:
...
$\hspace{0.5em} \text{a.} \quad$ Note the three relations meet at the boundaries, so we do not need to worry about specifying the points.
...
$\hspace{0.5em} \text{b.} \quad$ Be careful with the discontinuity at $x=0$ and $x=4$.
...
$\hspace{0.5em} \text{a.} \quad$ Note there are three separate regions in which the relations are defined: $x<0$, $0\le x < 4$, and $x\ge 4$.{{% sn q2a %}}Because there are no discontinuities, $x=0$ and $4$ can belong to either of the regions. For example, $x\le 0$, $0 < x \le 4$, and $x>4$ is also a valid division of the domain.{{% /sn %}}
...
$\hspace{0.5em} \text{b.} \quad$ There are four separate sub-domains for the functions. Focusing on the filled and open circles, the four regions are: $x\le -4$, $-4 < x < 0$, $0\le x \le 3$, and $x>3$.
...
$\hspace{0.5em} \text{a.} \quad$ The graph of $y=f(x)$ is below. Be careful with the discontinuity at $x=\dfrac{1}{2}$.

{{< figure src="courses/jpn-maths-1/functions/functions-and-graphs/piecewise-q3a.png" alt="Graph of y = f(x)." >}}

$\hspace{0.5em} \text{b.} \quad$ From the definition of $y = f(x)$,
$$ f(f(x)) = \begin{cases}
